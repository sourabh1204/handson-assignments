...
public class JdbcBackedDao implements AppDao {
	...
	@Override
	public Contact addContact(int profilId, Contact contact) {
		try {
			Connection connection = DBUtility.getConnection();
			PreparedStatement sequenceStatement = connection.prepareStatement("values(next value for profile_seq)");
			ResultSet rs = sequenceStatement.executeQuery();
			...
			PreparedStatement preparedStatement = connection.prepareStatement("insert into contact_table values(?,?,?,?)");
			preparedStatement.setInt(1, seq);
			preparedStatement.setString(2, contact.getContactName());
			preparedStatement.setLong(3, contact.getContactNo());
			preparedStatement.setInt(4, profilId);
			preparedStatement.executeUpdate();
			System.out.println("Contact created with an id = "+seq);
			preparedStatement.close();
			connection.close();
		} catch(ClassNotFoundException e) {
			e.printStackTrace();
	...
	public Contact getContact(int contactId) throws ContactNotFoundException {
		...
		try {
			Connection connection = DBUtility.getConnection();
			PreparedStatement preparedStatement = connection.prepareStatement("select * from contact_table where contact_id = ?");
